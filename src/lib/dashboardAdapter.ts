...
export function adaptOverview(data: OverviewApi | null | undefined) {
  ...
  return {
    ...
    stats: {
      ...
      avgOilLevel: Number(stats?.avg_oil_level ?? 0),
      avgKnotsVertical: Number(stats?.avg_knots_vertical ?? 0),
    ...
    materials: materials
      ...
      .map((m) => ({
        ...
        avgVolume: Number(m.avg_volume ?? 0),
        ...
        avgOilTemperature: Number(m.avg_oil_temperature ?? 0),
        avgOilLevel: Number(m.avg_oil_level ?? 0),
        avgKnotsVertical: Number(m.avg_knots_vertical ?? 0),
        avgHighPressure: Number(m.avg_high_pressure ?? 0),
        avgChannelPressure: Number(m.avg_channel_pressure ?? 0),
    ...
    timeline: {
      ...
      rows: timelineRows.map((r: any) => ({
        ...
        sumTotalTime: Number(r.sum_total_time ?? 0),
        sumAutoTime: Number(r.sum_auto_time ?? 0),
        sumStandbyTime: Number(r.sum_standby_time ?? 0),
        sumEmptyTime: Number(r.sum_empty_time ?? 0),
      })),
    },
  };
}